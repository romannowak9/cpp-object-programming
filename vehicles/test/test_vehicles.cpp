...
TEST(DriverAuxTest, assignVehicleToDriver) {
    ...
    vehicles.push_back(std::make_unique<Car>("car_1", "Opel", 100));
    vehicles.push_back(std::make_unique<Car>("car_2", "BMW", 200));
    vehicles.push_back(std::make_unique<Bicycle>("bike_1", "Kross", 5));
    Driver driver_1("Roman", Gender::Male);
    assign_vehicle_to_driver(vehicles, driver_1);

    ASSERT_EQ(vehicles.size(), 2U);
    EXPECT_EQ(vehicles[0]->get_id(), "car_1");
    EXPECT_EQ(vehicles[1]->get_id(), "car_2");
    EXPECT_EQ((driver_1.get_vehicle())->get_id(), "bike_1");
}